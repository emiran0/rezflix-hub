...
| Auth       | **Auth.js v5 (NextAuth)**                                     | Credentials provider maps cleanly onto our own username/password store; the Jellyfin step is a _link/verification_, not a login, so we don't need a provider for it. We own password hashing + the user table; Auth.js owns the session.                                                                                                  |
...
> Hub-owned username/password, and Jellyfin is a _link step_ rather than a login provider, so
...
always username + password.** Jellyfin is never a login provider; it is a _link step_ that
...
### 6.1 Route protection & role gating

Access control is enforced **server-side, per route**, via the guards in `lib/auth-guards.ts`
(`requireUser` / `requireGuest` / `requireRole` / `requireAdmin`). Each decodes the Auth.js JWT
through `auth()` and `redirect()`s when the requirement isn't met — call them at the top of a
protected Server Component, layout, or Server Action, close to the data.

- **No auth middleware in v1 (deliberate).** The Credentials provider's `authorize` pulls Prisma
  and bcrypt into `auth.ts`, which can't run in Next's edge middleware runtime; the standard
  workaround (a split edge-safe `auth.config`) is avoidable complexity at this scale. Per-segment
  server checks are also the stronger, currently-recommended boundary (middleware is easy to
  under-match). If a coarse pre-check is ever wanted, add an edge-safe split config then.
- **Personas → guards** (PRD §3): guest = `requireGuest` (login/signup); applicant/member/admin
  are `role`-gated with `requireRole(...)`. `member` is the Jellyfin-linked role; `admin` comes
  from `ADMIN_EMAILS` (§5.4). Wrong-role users are sent home; signed-out users to `/login`.
- Client components never gate access — they only render what the server already authorized.
